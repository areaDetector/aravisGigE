...
void aravisCamera::doConnect(Guard& G)
{
    ...
    const char *deviceID, *firmwareVersion;
    GWrapper<ArvCamera> cam;
    ArvDevice* dev;
    ArvGc* gcam;
    ...
    // don't modify any other members
    {
        ...
        // Set Serial Number and Firmware Version
        deviceID = arv_camera_get_device_id(cam);
        if (deviceID) status |= setStringParam (ADSerialNumber, deviceID);
        firmwareVersion = arv_device_get_string_feature_value(dev, "DeviceFirmwareVersion");
