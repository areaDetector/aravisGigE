...
/* JR - add gv device private interface to get IP address of host */
...
/* Pulnix RM-6470 stream address must be set as a register write, just before starting acquisition */

void pulnix_set_stream_address(ArvCamera * camera)
{
    ...
    unsigned long * interface_bytes = (unsigned long *)g_inet_address_to_bytes(interface_address);
    unsigned long hostip = htonl(*interface_bytes);
    arv_device_write_register(arv_camera_get_device(camera), ARV_GVBS_FIRST_STREAM_CHANNEL_IP_ADDRESS, hostip);
}
...
main (int argc, char **argv)
{
	...
	if (!g_option_context_parse (context, &argc, &argv, &error)) {
        ...
        //FILE * pip = popen("dls-python2.4 -u viewer.py", "w");
        //assert(pip);
				...
				if (buffer != NULL) {
					if (buffer->status == ARV_BUFFER_STATUS_SUCCESS)
						...
						buffer_count++;
                                                ...
                                                char filename[1024];
                                                snprintf(filename, sizeof(filename), "frame%d.raw", buffer_count);
                                                FILE * f = fopen(filename, "w");
                                                /*
                                                if(buffer_count % 100 == 0)
                                                {
                                                    fwrite(buffer->data, sizeof(char), payload, pip);
                                                }
                                                */
                                                fwrite(buffer->data, sizeof(char), payload, f);
                                                //fflush(pip);
                                                fclose(f);
                                        }
